...
import { deployCampaignSale, getCurrentTimeInSeconds, daysToSeconds, CampaignParams, launchCampaign } from "./utils/funcs";

async function verifyCampaign(campaignSale: Contract, id: number, params: CampaignParams) {
...
describe("Get campaign", function () { 
  ...
  it("should fail for second ID after creating a campaign", async function () {
    await expect(
        this.campaignSale.getCampaign(2)
    ).to.be.revertedWith("campaign does not exist")
  });
